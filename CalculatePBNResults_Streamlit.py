...
# streamlit program to display statistics from a PBN file.

import streamlit as st
import pathlib
import pandas as pd
import polars as pl # todo: replace pandas with polars
...
def calculate_dd_tricks_pars_scores(df, progress=None):
    ...
    par_df = pd.DataFrame({'Par_Score_NS': par_scores_ns, 'Par_Score_EW': par_scores_ew, 'Par_Contract': par_contracts})
...
def calculate_sd_probs(df, sd_productions=100, progress=None):
    sd_cache_d = {}
    deals = df['deal'].astype(str) # using str of deal here
...
def calculate_sd_expected_values(df,sd_cache_d,scores_d):
    ...
    deal_vul = zip(df['deal'].astype(str),df['_vul']) # using str of deal here
...
def create_augmented_df(merged_df):
    cols = ['board_num','deal','Room','_contract','Score','_vul','Par_Contract','Par_Score_NS','Exp_Max_Col','Exp_Max','Exp_Max_NS','Best_Contract','North','East','South','West']
    ...
    augmented_df['Par_Diff_NS'] = augmented_df['Score_NS']-augmented_df['Par_Score_NS'].astype('int16')
...
if __name__ == '__main__':
    ...
    sd_productions_default = 10  # number of random deals to generate for calculating single dummy probabilities. Use smaller number for testing.

    # Streamlit UI
    st.title("Calculate PBN Results")
    app_info()

    # File uploader
    uploaded_file = st.file_uploader("Choose a PBN file", type="pbn", )
    if uploaded_file is not None:
        pbn_file = pathlib.Path(uploaded_file.name)
        ...
        df = parse_pbn_file(boards)
        st.caption("Information Dataframe")
        # Other dataframe components don't do implicit str conversion like pd.DataFrame. Must manually convert object columns to strings.
        str_df = df.copy()
        str_df['deal'] = str_df['deal'].astype(str)
        str_df['auction'] = str_df['auction'].map(lambda x: ', '.join(map(str,x[:3]))+' ...')
        str_df['play'] = str_df['play'].map(lambda x: ', '.join(map(str,x[:3]))+' ...')
        str_df['_contract'] = str_df['_contract'].astype(str)
        streamlitlib.ShowDataFrameTable(str_df, key='info_df')
        ...
        streamlitlib.ShowDataFrameTable(dd_tricks_df, key='dd_tricks_df')
        st.caption("Par Scores Dataframe")
        streamlitlib.ShowDataFrameTable(par_df, key='par_df')
        st.caption("Double Dummy Scores Dataframe")
        streamlitlib.ShowDataFrameTable(dd_score_df, key='dd_score_df')
        ...
        streamlitlib.ShowDataFrameTable(sd_probs_df, key='sd_probs_df')

        scores_d, sd_scores_df = calculate_sd_scores()
        st.caption("Single Dummy Scores Dataframe")
        streamlitlib.ShowDataFrameTable(sd_scores_df, key='sd_scores_df')

        sd_expected_values_df = calculate_sd_expected_values(df, sd_cache_d, scores_d)
        st.caption("Expected Values Dataframe")
        streamlitlib.ShowDataFrameTable(sd_expected_values_df, key='sd_expected_values_df')

        sd_best_contract_df = calculate_best_contracts(sd_expected_values_df)
        st.caption("Best Contracts Dataframe")
        streamlitlib.ShowDataFrameTable(sd_best_contract_df, key='sd_best_contract_df')

        merged_df = pd.concat([str_df,par_df,dd_tricks_df,dd_score_df,sd_best_contract_df],axis='columns')
        augmented_df = create_augmented_df(merged_df)
        streamlitlib.ShowDataFrameTable(augmented_df, key='augmented_df')
